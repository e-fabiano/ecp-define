...
function set_active_atoms {
    atom_type="$1"
    active_atom_list=""
    for i in $(seq $natoms); do
        j=$(echo "$i + 1" | bc)
        atom=$(echo "$coords" | head -n $j | tail -n 1 | awk '{print $4}')
	if [ "$atom" == "$atom_type" ]; then
	    ch=0
	    freeze_list=$(echo "$freeze" | sed s/,/" "/g)
	    for k in $freeze_list; do
		if [ "$k" == "$i" ]; then
		    ch=1
		    break
		fi
	    ...
	    if [ "$ch" == "0" ]; then
		active_atom_list=$(echo "$active_atom_list,$i")
	    fi
	fi
    done
    if [ "$active_atom_list" != "" ]; then
	active_atom_list="${active_atom_list:1}"
    fi
}
...
#
	...
	set_active_atoms "$at"
	if [ "$active_atom_list" != "" ]; then
	    echo "ecp $active_atom_list ccecp-${basis}-ecp" >> def.in
	fi
    done
    if [ "$ish" == "yes" ]; then
